...
- **Completion** is confirmed through a modal (optional note, optional Just/Suus override defaulting to the current user; for a one-off it notes the task will be archived); task cards carry a green→orange→red left-border gradient for time-until-due.
